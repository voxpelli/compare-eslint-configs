...
- **`lib/audit.js`** — `auditConfig()` builds an `AuditContext` (configuredRules, pluginNames, pluginRuleMeta) then runs three named check functions: `checkDeprecatedBuiltins` (ESLint core rules), `checkDeprecatedPluginRules` (plugin rules with `meta.deprecated` — iterates `rawConfigs[].plugins[prefix].rules`), `checkUnconfiguredBuiltins`. Results flow to `printAuditResult` or JSON. `builtinRules` comes from `eslint/use-at-your-own-risk`
