...
function formatConfigSummaryList (sortedRules, { markdown, skipLinks, verboseConfigs }) {
  ...
  for (const [
    ruleName,
    { docUrls, error, isDeprecated, options },
  ] of sortedRules) {
    ...
    formattedRules.push([
      ...
      isDeprecated ? format.dim(' [deprecated]') : '',
...
function formatConfigSummaryTable (sortedRules, { markdown, skipLinks, verboseConfigs }) {
  ...
  for (const [
    ruleName,
    { docUrls, error, isDeprecated, options: config },
  ] of sortedRules) {
    const docUrl = [...(docUrls || [])][0];
    const optionsValue = config && Object.values(config)?.[0];

    tableData.push([
      isDeprecated ? [mdastLinkify(ruleName, docUrl, skipLinks), format.dim(' [deprecated]')] : mdastLinkify(ruleName, docUrl, skipLinks),
      error?.length ? 'Yes' : '',
      error?.length ? '' : 'Yes',
