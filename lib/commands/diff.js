...
async function setupCommand (name, description, args, { pkg }) {
  const options = /** @satisfies {import('peowly').AnyFlags} */ ({
    ...
    markdown: outputFlags.markdown,
    'no-links': outputFlags['no-links'],
    'verbose-configs': outputFlags['verbose-configs'],
    'exit-code': {
  ...
  const {
    ...
    options,
    pkg,
    usage: '[<baseline config>] <new config>',
  });
  ...
  if (configFiles.length === 1) {
    ...
    if (configFiles[0] && path.resolve(configFiles[0]) === path.resolve(defaultConfig)) {
...
/**
 * @param {Pick<import('./command-types.d.ts').CommandContextDiff, 'configFiles' | 'configs' | 'targetFile' | 'verbose'>} context
 * @returns {Promise<false|import('../compare.js').ConfigDiff>}
 */
async function doTheWork (context) {
  const { configFiles, configs, targetFile, verbose } = context;
...
function formatWorkResult (diffResult, context) {
  ...
  if (jsonOutput) {
    ...
    console.log(JSON.stringify(diffResult, undefined, 2));
  } else {
