...
import { sortRuleCollectionByName } from './utils.js';
...
export function printDiffResult (diffResult, options) {
  ...
  if (Object.keys(diffResult.added).length) {
    ...
    console.log(format.fromMdast(formatRuleList(sortRuleCollectionByName(diffResult.added), diffResult.ruleDocs, options)));
  }

  if (Object.keys(diffResult.removed).length) {
    console.log(format.header('Removed'));
    console.log(format.fromMdast(formatRuleList(sortRuleCollectionByName(diffResult.removed), diffResult.ruleDocs, options)));
  }

  if (Object.keys(diffResult.changedSeverity).length) {
    console.log(format.header('Changed severity'));
    console.log(format.fromMdast(formatRuleList(sortRuleCollectionByName(diffResult.changedSeverity), diffResult.ruleDocs, options)));
  }

  if (Object.keys(diffResult.changedConfig).length) {
    console.log(format.header('Changed config'));
    console.log(format.fromMdast(formatRuleList(sortRuleCollectionByName(diffResult.changedConfig), diffResult.ruleDocs, options)));
  }
}
