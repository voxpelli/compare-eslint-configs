...
export function printDiffResult (diffResult, options) {
  ...
  if (Object.keys(diffResult.added).length) {
    ...
    console.log(format.fromMdast(formatRuleList(sortRulesByName(diffResult.added), diffResult.ruleDocs, options, diffResult.deprecated)));
  }

  if (Object.keys(diffResult.removed).length) {
    console.log(format.header('Removed'));
    console.log(format.fromMdast(formatRuleList(sortRulesByName(diffResult.removed), diffResult.ruleDocs, options, diffResult.deprecated)));
  }

  if (Object.keys(diffResult.changedSeverity).length) {
    console.log(format.header('Changed severity'));
    console.log(format.fromMdast(formatRuleList(sortRulesByName(diffResult.changedSeverity), diffResult.ruleDocs, options, diffResult.deprecated)));
  }

  if (Object.keys(diffResult.changedConfig).length) {
    console.log(format.header('Changed config'));
    console.log(format.fromMdast(formatRuleList(sortRulesByName(diffResult.changedConfig), diffResult.ruleDocs, options, diffResult.deprecated)));
  }
}
 ...
 * @param {PrintOptions} options
 * @param {import('./compare.js').ConfigDiff["deprecated"]} [deprecated]
 * @returns {import('mdast').List}
 */
function formatRuleList (rules, ruleDocs, { markdown, skipLinks, verboseConfigs }, deprecated) {
  ...
  for (const [ruleName, ruleConfig] of Object.entries(rules)) {
    ...
    formattedRules.push([
      ...
      deprecated?.[ruleName] ? format.dim(' [deprecated]') : '',
