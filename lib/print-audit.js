...
export function printAuditResult (result, { markdown, showUnconfigured, skipLinks }) {
  ...
  const statsLines = [
    ...
    '',
    `  Configured rules: ${result.stats.totalConfigured} (${result.stats.totalBuiltinsConfigured} builtin, ${result.stats.totalConfigured - result.stats.totalBuiltinsConfigured} plugin)`,
    `  Deprecated in use: ${result.stats.totalDeprecated}`,
    `  Builtin coverage: ${result.stats.totalBuiltinsConfigured} / ${result.stats.totalBuiltinsActive} active builtins (${result.stats.totalBuiltinsActive > 0 ? Math.round(result.stats.totalBuiltinsConfigured / result.stats.totalBuiltinsActive * 100) : 0}%)`,
      ...
      if (rule.replacedBy.length > 0) {
        for (const replacement of rule.replacedBy) {
          ...
          const replDisplay = replUrl && !skipLinks
            ? format.hyperlink(replId, replUrl, { fallbackToUrl: true })
            : replId;
