 ...
 * @param {{ targetFile?: string | undefined, verbose?: boolean | undefined }} [options]
 * @returns {{ [ruleName: string]: RuleSummary}}
 */
export function summarizeConfigs (configs, { targetFile, verbose } = {}) {
  ...
  for (const [configName, { config, engine }] of Object.entries(configs)) {
    ...
    for (const ruleId in rulesMeta) {
      ...
      const deprecated = rulesMeta[ruleId]?.deprecated;
      if (deprecated) {
        foundRule.isDeprecated = true;
        if (typeof deprecated === 'object' && !foundRule.deprecation) {
          foundRule.deprecation = deprecated;
        }
      }
    }
  }
 ...
 * @property {{ [ruleName: string]: boolean }} deprecated
 */

/**
 * @param {{ [configName: string]: Config }} configs
 * @param {{ targetFile?: string | undefined }} [options]
 * @returns {ConfigDifferences}
 */
export function compareConfigs (configs, { targetFile } = {}) {
  const configCount = Object.keys(configs).length;
  const summarizedRules = summarizeConfigs(configs, { targetFile });
  ...
  const ruleDocs = {};
  /** @type {ConfigDifferences["deprecated"]} */
  const deprecated = {};

  for (const [ruleName, summary] of Object.entries(summarizedRules)) {
    const { docUrls, error, isDeprecated, options, warn } = summary;
 ...
 */
export function diffConfigs (target, source, { targetFile, verbose = false }) {
  ...
  for (const [ruleName, summary] of Object.entries(summarizedRules)) {
    const { docUrls, error, isDeprecated, overridenOptions, warn } = summary;
