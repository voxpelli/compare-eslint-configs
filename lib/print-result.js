 ...
 * @property {boolean} verboseConfigs
 */

/**
 * @param {import('./compare.js').ConfigDifferences} differences
 * @param {string[]} configNames
 * @param {Omit<PrintOptions, 'table' | 'verboseConfigs'> & { groupByRule: boolean }} options
 * @returns {void}
 */
export function printComparationResult (differences, configNames, options) {
  ...
  if (onlyActiveInKeys.length) {
    ...
    if (groupByRule) {
      for (const ruleName of onlyActiveInKeys) {
        ...
        if (items.length) {
          const deprecatedTag = deprecated[ruleName] ? format.dim(' [deprecated]') : '';
          console.log(format.header(
            `${format.hyperlink(format.bold(ruleName), skipLinks ? undefined : (ruleDocs[ruleName] || [])[0], { fallback: false })}${deprecatedTag} only active in:`,
            2
          ));
      ...
      for (const configName of configNames) {
        ...
        for (const ruleName of onlyActiveInKeys) {
          if (onlyActiveIn[ruleName]?.includes(configName)) {
            const deprecatedTag = deprecated[ruleName] ? format.dim(' [deprecated]') : '';
            innerItems.push(format.hyperlink(ruleName, skipLinks ? undefined : (ruleDocs[ruleName] || [])[0], { fallback: false }) + deprecatedTag);
          }
        }
  ...
  if (mixedSeverityEntries.length) {
    ...
    for (const [ruleName, severities] of mixedSeverityEntries) {
      ...
      const deprecatedTag = deprecated[ruleName] ? format.dim(' [deprecated]') : '';
      items.push(format.hyperlink(format.bold(ruleName), skipLinks ? undefined : (ruleDocs[ruleName] || [])[0], { fallback: false }) + deprecatedTag + '\n' + format.list(formattedSeverities).trimEnd());
    }
