...
function collectPluginRuleMeta (rawConfigs) {
  ...
  for (const configObj of rawConfigs) {
    ...
    for (const [prefix, plugin] of Object.entries(plugins)) {
      const rules = /** @type {Record<string, unknown> | undefined} */ (
        /** @type {Record<string, unknown>} */ (plugin || {})['rules']
      );
      if (!rules) continue;

      for (const [ruleName, rule] of Object.entries(rules)) {
        const meta = /** @type {Record<string, unknown> | undefined} */ (
          /** @type {Record<string, unknown>} */ (rule || {})['meta']
        );
        if (meta) {
...
function checkDeprecatedPluginRules ({ configuredRules, pluginRuleMeta }) {
  ...
  for (const [fullRuleName, { meta }] of pluginRuleMeta) {
    ...
    if (typeof deprecated === 'object' && Array.isArray(
      /** @type {Record<string, unknown>} */ (deprecated)['replacedBy']
    )) {
      for (const replacement of /** @type {Array<Record<string, unknown>>} */ (
        /** @type {Record<string, unknown>} */ (deprecated)['replacedBy']
      )) {
        const replacementRule = /** @type {Record<string, unknown> | undefined} */ (replacement['rule']);
        const replacementPlugin = /** @type {Record<string, unknown> | undefined} */ (replacement['plugin']);
        replacedBy.push({
          ruleId: /** @type {string | undefined} */ (replacementRule?.['name']),
          pluginName: /** @type {string | undefined} */ (replacementPlugin?.['name']),
          message: /** @type {string | undefined} */ (replacement['message']),
          url: /** @type {string | undefined} */ (replacement['url']),
        });
      }
    ...
    const docs = /** @type {Record<string, unknown> | undefined} */ (meta['docs']);

    results.push({
      ruleName: fullRuleName,
      severity: configuredRules.get(fullRuleName) || 'error',
      replacedBy,
      docUrl: /** @type {string | undefined} */ (docs?.['url']),
      deprecatedSince: typeof deprecated === 'object'
        ? /** @type {string | undefined} */ (/** @type {Record<string, unknown>} */ (deprecated)['deprecatedSince']) ?? undefined
        : undefined,
      availableUntil: typeof deprecated === 'object'
        ? /** @type {string | undefined} */ (/** @type {Record<string, unknown>} */ (deprecated)['availableUntil']) ?? undefined
        : undefined,
    });
  }
