...
export async function findDefaultConfig (cwd) {
  ...
  const configPath = await engine.findConfigFile();

  if (!configPath) {
    throw new InputError('No ESLint flat config found in current directory (eslint.config.js/.mjs/.cjs/.ts). Pass a config file explicitly.');
  }

  return path.relative(cwd, configPath);
}

export const inputFlags = /** @satisfies {Record<string, import('peowly').AnyFlag & { listGroup: 'Input options' | 'Deprecated' }>} */ ({
  'for-file': {
    description: 'File to compute the effective rule set for',
  ...
  'target-file': {
    description: '(deprecated: use --for-file)',
    listGroup: 'Deprecated',
    'short': 't',
    type: 'string',
  },
});
...
export async function resolveInputContext (flags, configFiles) {
  let targetFile = flags['for-file'];
  if (flags['target-file'] !== undefined) {
    process.emitWarning('--target-file (-t) is deprecated, use --for-file (-f)', 'DeprecationWarning');
    targetFile = flags['target-file'];
  }
  ...
  if (!targetStat || !targetStat.isFile()) {
    ...
    throw new InputError(`Can't find "${targetAbsolute}", set a target with --for-file`);
  }
...
async function resolveEngine (configFile) {
  ...
  // eslint-disable-next-line security/detect-non-literal-fs-filename
  const configStat = await stat(configFileAbsolute).catch(() => {});
