...
/**
 * @param {Partial<import('peowly').TypedFlags<typeof outputFlags>>} flags
 * @returns {import('./flag-types.d.ts').OutputFlags}
 */
export function resolveOutputFlags (flags) {
  const {
    'group-rules': groupByRule = false,
    json: jsonOutput = false,
    markdown: markdownOutput = false,
    'no-links': skipLinks = false,
    table = false,
    'verbose-configs': verboseConfigs = false,
  } = flags;
